 ...
 * @subpackage Patterns_Fitness/Patterns_Fitness_Api_Settings
 */
 ...
 *
 * @see Patterns_Fitness_Api
 */
if ( ! class_exists( 'Patterns_Fitness_Api_Settings' ) ) {

	/**
	 * Patterns_Fitness_Api_Settings
	 *
	 * @see WP_REST_Settings_Controller
	 * @package Patterns_Fitness
	 * @since 1.0.0
	 */
	class Patterns_Fitness_Api_Settings extends Patterns_Fitness_Api {
		 ...
		 */
		public function run() {
			$this->type      = 'patterns_fitness_api_settings';
		...
		public function get_item( $request ) {
			...
			$saved_options = patterns_fitness_get_options();
		...
		public function update_item( $request ) {
			...
			if ( is_wp_error( rest_validate_value_from_schema( $params, $schema ) ) ) {
				return new WP_Error(
					...
					sprintf( __( 'The %s property has an invalid stored value, and cannot be updated to null.', 'patterns-fitness' ), PATTERNS_FITNESS_OPTION_NAME ),
					array( 'status' => 500 )
				);
			}
			...
			/* Update options */
			patterns_fitness_update_options(
				array(
			...
			/* Update user meta */
			patterns_fitness_update_user_meta(
				get_current_user_id(),
				array(
		...
		public function get_settings_schema() {
			$setting_properties = apply_filters(
				'patterns_fitness_options_properties',
				array(
...
/**
 * Return instance of  Patterns_Fitness_Api_Settings class
 *
 * @since 1.0.0
 *
 * @return Patterns_Fitness_Api_Settings
 */
function patterns_fitness_api_settings() { //phpcs:ignore
	return Patterns_Fitness_Api_Settings::get_instance();
}
patterns_fitness_api_settings()->run();
