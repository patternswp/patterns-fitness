/*CSS*/
/* global PatternsFitnessLocalize */
...
/* Local */
class PatternsFitnessSettingsApi {
	constructor() {
		this.endpoint = 'patterns-fitness/v1/settings';
	...
	bindEvents() {
		document.addEventListener( 'click', ( event ) => {
			...
			if ( target.closest( '.patterns-fitness-gsn-close' ) ) {
				this.modifySettings( 'hide_get_started_notice', true );
			} else if (
				target.closest( '.patterns-fitness-rn-permanent-close' )
			) {
				this.modifySettings( 'remove_review_notice_permanently', true );
			} else if (
				target.closest( '.patterns-fitness-rn-temporary-close' )
			) {
				this.modifySettings(
				...
				const installBtn = target.closest(
					'.patterns-fitness-install-plugins'
				);
				if ( installBtn ) {
					installBtn.classList.add( 'patterns-fitness-processing' );
					this.installPlugins();
				}
			}
		} );
	...
	updateUI() {
		...
		if ( ! this.isLoadReview() ) {
			const reviewNoticeElement = document.getElementById(
				'patterns-fitness-review-notice'
			);
	...
	async installPlugins() {
		const plugins = PatternsFitnessLocalize.recommended_plugins;
		...
		window.location.replace( PatternsFitnessLocalize.theme_info_url );
	}
}

document.addEventListener( 'DOMContentLoaded', () => {
	new PatternsFitnessSettingsApi();
} );
